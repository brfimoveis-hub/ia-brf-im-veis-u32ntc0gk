...
migrate(
  (app) => {
    ...
    const updatedBiaPrompt = `Você é a Bia, assistente virtual da BRF Imóveis (www.brfimoveis.com.br).
...
- Quando o cliente perguntar "quem é você?", "de onde veio seu nome?", "você é um robô?", "é IA?" ou questionar sua identidade:
...
- Quando o cliente solicitar expressamente um corretor humano, visita presencial com o corretor responsável, agendamento de avaliação presencial ou negociação comercial direta: faça o direcionamento cordial para o Mauro Fengler via WhatsApp oficial: https://wa.me/5548992098050 e inclua a tag [HANDOVER: Mauro].`
    ...
    try {
      ...
      log.set('type', 'ai_prompt_identity_alignment')
      log.set(
        'message',
        'Identificação da Bia atualizada para padrão de mercado ("Bia, assistente virtual da BRF Imóveis").',
      )
      log.set(
        'details',
        'Proibida menção a detalhes de cadastros/CRM ou campos internos ao justificar nomes ou identidade.',
      )
